...
g1 = ggplot(df85_1, aes(y = mean, x = y, color = gear)) +
  ...
  labs(y = expression(paste('Mean ',Delta,'2100-2014')), x = 'Latitude', title = 'RCP8.5') +
  scale_x_continuous(breaks = c(-90,-60,-30,0,30,60,90)) +
  scale_y_continuous(breaks = c(-0.1,0,0.1,0.2,0.3,0.4), limits = c(-0.1,0.4)) +
...
g2 = ggplot(df26_1, aes(y = mean, x = y, color = gear)) +
  geom_hline(yintercept = 0) +
  geom_line(linewidth = 0.6) +
  labs(y = expression(paste('Mean ',Delta,'2100-2014')), x = 'Latitude', title = 'RCP2.6', color = 'Fishing gear') +
  scale_x_continuous(breaks = c(-90,-60,-30,0,30,60,90)) +
  scale_y_continuous(breaks = c(-0.1,0,0.1,0.2,0.3,0.4), limits = c(-0.1,0.4)) +
  scale_color_manual(values = color_pallet) +
  guides(color = 'none') +
...
g3 = ggplot(df85, aes(x=round(((area_f85/45234)-(area_p/45234))*100,2), y=norm, fill = gear)) +
  ...
  labs(x = expression(paste(Delta,'Ocean fished (%)')), y = 'Weighted net change',
       title = 'RCP8.5', size = 'Current ocean fished (%)', fill = 'Fishing gear') +
...
g4 = ggplot(df26, aes(x=round((((area_f26/45234)-(area_p/45234))*100),2), y=norm, fill = gear)) +
  geom_hline(yintercept = 0, linetype = 3) +
  geom_vline(xintercept = 0, linetype = 3) +
  geom_errorbar(aes(ymin = norm_low, ymax = norm_up), width = 0, color = 'grey') +
  geom_point(aes(size = round((area_p/45234)*100)), shape = 21) +
  ...
  labs(x = expression(paste(Delta,'Ocean fished (%)')), y = 'Weighted net change',
       title = 'RCP2.6', size = 'Current ocean fished (%)', fill = 'Fishing gear') +
  scale_fill_manual(values = color_pallet) +
  scale_color_manual(values = color_pallet) +
  geom_text_repel(aes(label = paste0(round(change),'%'), color=gear),
                   box.padding   = 0.25,
...
png(paste0(path,'/figures/Figure 2A-D.png'),res = 800,units = 'in',height = 6, width = 11.6)
