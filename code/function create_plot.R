...
create_plot <- function(df, title, limits){
  ...
  g <- ggplot() +
    ...
    geom_path(data=eez,aes(x=long,y=lat,group=group), color='grey50', linewidth = 0.3, inherit.aes = FALSE) +
    geom_polygon(data=wrld_simpl,aes(x=long,y=lat,group=group),fill='lightgrey',color='grey') +
...
create_plot_beta <- function(df, title, limits){
  ...
  g <- ggplot() +
    geom_spatraster(data=df_raster) +
    geom_path(data=eez,aes(x=long,y=lat,group=group), color='lightgrey', linewidth = 0.3, inherit.aes = FALSE) +
    geom_polygon(data=wrld_simpl,aes(x=long,y=lat,group=group),fill='lightgrey',color='grey') +
