...
df85$area_perc <- round((df85$area/45234)*100,2)
df26$area_perc <- round((df26$area/45234)*100,2)
...
g1 = ggplot(df85 %>%
  ...
  labs(x = 'Current ocean fished (%)', y = 'Weighted net change', title = 'RCP8.5', fill = '') +
...
g2 = ggplot(df26 %>%
  ...
  scale_y_continuous(limits = c(-0.07,0.06)) +
  scale_x_continuous(limits = c(0,40), breaks = seq(0,40,10)) +
  labs(x = 'Current ocean fished (%)', y = 'Weighted net change', title = 'RCP2.6', fill = '') +
  scale_fill_manual(values = color_pallet) +
  geom_label_repel(aes(label = paste0(code,' (',round(change),'%)')),
                   box.padding   = 0.25,
                   size = 2.7,
...
png(paste0(path,'/figures/Figure 3A-B.png'),res = 600,units = 'in',height = 4.5, width = 10.5)
